...
use serenity::{
    futures::StreamExt,
    model::{
        prelude::{ChannelId, GuildChannel, GuildId, Reaction, ReactionType},
        user::User,
    },
    prelude::Context,
};
...
pub async fn handle(handler: &Handler, ctx: Context, add_reaction: Reaction) {
    ...
    let entry = sqlx::query!(
        "SELECT Id 
        FROM Votes 
        WHERE IsApprovedByAuthor=0 AND ApprovedByAdminId=0 AND MessageID=? AND ServerID=? AND AuthorID=?",
        message_id,
        server_id,
        author_id
    )
...
async fn send_to_verification(
    ...
    let verification_message_id = i64::try_from(sent_message_id.as_u64().to_owned()).unwrap();
    sqlx::query!(
        "UPDATE Votes
            SET IsApprovedByAuthor=1, VerificationMessageID=?
            WHERE Id=? AND ServerID=?",
